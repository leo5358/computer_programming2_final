...
const PLAYER_SHEET: Texture2D = preload("res://assets/sprites/player/player_sheet.png")
...
func _update_movement(delta: float) -> void:
	...
	if state == PlayerState.DASH:
		velocity.x = facing * dash_impulse
		return
...
func _update_visuals() -> void:
	attack_visual.visible = state == PlayerState.ATTACK
	...
	parry_visual.visible = is_parrying or parry_flash_timer > 0.0
	block_impact_visual.visible = block_flash_timer > 0.0
	hurt_impact_visual.visible = hurt_flash_timer > 0.0
...
func _setup_sprite_frames() -> void:
	...
	_add_sheet_animation(frames, "run", 1, 8, 12.0, true)
	_add_sheet_animation(frames, "attack_a", 2, 12, 24.0, false)
	_add_sheet_animation(frames, "parry", 3, 6, 18.0, false)
	_add_sheet_animation(frames, "block", 4, 4, 8.0, true)
	_add_sheet_animation(frames, "dash", 5, 6, 20.0, false)
	_add_sheet_animation(frames, "jump", 6, 6, 8.0, true)
	_add_sheet_animation(frames, "hurt", 7, 4, 10.0, false)
	_add_sheet_animation(frames, "death", 7, 8, 8.0, false)
