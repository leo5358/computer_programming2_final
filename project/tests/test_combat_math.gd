...
func _initialize() -> void:
	...
	_assert_close(math.add_heartbeat(110.0, 20.0), 130.0, "heartbeat can rise beyond the old high-tension point")
	_assert_close(math.add_heartbeat(190.0, 20.0), 200.0, "heartbeat clamps at death threshold")
	...
	_assert_close(math.block_duration_for_heartbeat(200.0), 0.2, "lethal heartbeat barely allows held block")
	_assert_close(math.damage_with_adrenaline(20.0, 120.0), 30.0, "high heartbeat increases damage")
	_assert_close(math.damage_with_adrenaline(20.0, 200.0), 30.0, "lethal heartbeat should not add runaway damage")
	...
	_assert_close(math.posture_damage_with_adrenaline(20.0, 200.0), 28.0, "lethal heartbeat should not add runaway posture pressure")
	quit(0)
