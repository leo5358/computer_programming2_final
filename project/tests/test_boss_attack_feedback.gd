...
func _initialize() -> void:
	...
	if attack_visual.visible:
		push_error("Boss attack cue should not show gold before the perfect parry window")
		quit(1)
		return
	for frame in 40:
		await physics_frame
	...
	if not attack_visual.visible:
		push_error("Boss attack cue should show AttackVisual when the perfect parry window opens")
		quit(1)
		return
	...
	if attack_shape.size.x > 134.0:
		push_error("Boss normal attack hitbox should stay within the manually calibrated weapon reach")
		quit(1)
		return
	if attack_shape.size != Vector2(90.0, 80.0):
		push_error("Boss normal attack hitbox should keep the manually calibrated size")
		quit(1)
		return
	if attack_area.position != Vector2(20.0 * boss.facing, -36.0):
		push_error("Boss normal attack hitbox should keep the manually calibrated offset")
		quit(1)
		return
